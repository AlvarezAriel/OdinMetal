...
float2 map( float3 q, float iTime, constant Uniforms *uni ) {
    // plane
    float2 res = float2( q.y, 2.0 );
    
    // sphere
    float d = length(q-float3(-uni->cursor.x + 0.5,-uni->cursor.y + 0.5,0.0))-0.02;
    
    float d2 = length(q-float3(0.,0.,0.0))-0.03;

    // smooth union    
    float2 m1 = smin(res.x,d,0.05);
    float2 m2 = smin(d2,d,0.05);

    return min(m1, m2);
}

float2 intersect(float3 ro, float3 rd, float iTime, constant Uniforms *uni) {
    ...
    for( int i=0; i<512; i++ )
    {
	    float2 h = map( ro+rd*t, iTime , uni);
...
float3 calcNormal(float3 pos, float iTime, constant Uniforms *uni ) {
    float2 e = float2(1.0,-1.0)*0.5773*0.005;
    return normalize( e.xyy*map( pos + e.xyy, iTime, uni ).x + 
					  e.yyx*map( pos + e.yyx, iTime, uni ).x + 
					  e.yxy*map( pos + e.yxy, iTime, uni ).x + 
					  e.xxx*map( pos + e.xxx, iTime, uni ).x );
}

// https://iquilezles.org/articles/nvscene2008/rwwtt.pdf
float calcAO(float3 pos, float3 nor, float ran, float iTime, constant Uniforms *uni ) {
	float ao = 0.0;
    const int num = 32;
    for ( int i=0; i<num; i++ ) {
        float3 ap = forwardSF( float(i)+ran, float(num) );
		ap *= sign( dot(ap,nor) ) * hash1(float(i));
        ao += clamp( map( pos + nor*0.01 + ap*0.2, iTime, uni ).x*20.0, 0.0, 1.0 );
    }
	ao /= float(num);
...
float3 render(float2 p, float4 ran, float iTime, constant Uniforms *uni) {
    //-----------------------------------------------------
    // camera
    //-----------------------------------------------------
	float an = 0.1;
	...
	// raymarch
    float3 uvw;
    float2 res = intersect(ro,rd, iTime, uni);
    float t = res.x;
    if( t > 0.0 ) {
        float3 pos = ro + t*rd;
        float3 nor = calcNormal(pos, iTime, uni);
		float3 ref = reflect( rd, nor );
        float fre = clamp( 1.0 + dot(nor,rd), 0.0, 1.0 );
        float occ = calcAO( pos, nor, ran.y, iTime, uni ); occ = occ*occ;
  ...
  //float2 uv = (vert.position.xy - .5 * uni->screen_size.x) / uni->screen_size.y;
    ...
    for( int m=0; m<AA; m++ ) {
        for( int n=0; n<AA; n++ ) {
            ...
            col += render(p, float4(1.0), iTime, uni);    
        }
    }
